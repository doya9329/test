...
// 키입력 > KeyListener > KeyEvent 키보드 입력 관련 이벤트 처리 
public class KeyMouseExam extends JFrame 
	implements KeyListener, MouseListener, MouseMotionListener {
	//필드 변수
	private int x,y; //이미지의 x,y 좌표
	...
	public KeyMouseExam() { //생성자
		...
		setDefaultCloseOperation(EXIT_ON_CLOSE); //윈도우 닫기 버튼 동작
	}
	//컴포넌트를 화면에 출력하는 method
	//Graphics 그래픽 처리 객체
	@Override
	public void paint(Graphics g) {
		super.paint(g); // JFrame의 paint 호출
		width=img.getWidth(this); //이미지의 가로 사이즈 계산
		height=img.getHeight(this); //이미지의 세로 사이즈 계산
		//이미지를 화면에 출력  drawImage(이미지,x좌표,y좌표,이미지관찰자)
		//이미지관찰자 : 이미지에 대한 다양한 처리를 지원하는 객체
